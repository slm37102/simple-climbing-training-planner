...
- `node "$WT/tools/generate-sw.mjs"` when anything under `js/` or `css/` changed. CI hard-fails if `sw.js` wasn't regenerated. The `CACHE` key is a content hash of the shell (IB-055), not a hand-bumped counter, so regeneration is **idempotent** — re-run it freely (e.g. after a `/code-review` round changes `js/` again); with no source change it's a no-op, and it can't collide with a parallel pass's key.
