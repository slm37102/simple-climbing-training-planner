...
**Any trip-wire — it's a multi-session build, and the decision comes first.** A change to prescription math can't be built inline; it needs a human decision, then a spec, then tickets, then a PR. Instead of leaving that as a dead "routed to multi-session" note that never moves, **append the item to the grill queue** (`docs/grill-queue.md`) as a `Queued` row: its ID, a one-line *decision to settle*, priority, and a pointer back to its `IB-*` row. That queue is worked one item at a time by **`/grill-queue`** (which runs the interactive `/grilling` + `/domain-modeling` grill — both invocable — and graduates the finding to an ADR + `KG-*` decision). So an agent's job on a trip-wire item is: record the routing on the `IB-*` row, sharpen the finding in place, **enqueue it for grilling**, then tell the user it's queued and they can run `/grill-queue` to decide it. It does **not** edit the trip-wired code inline, and it does **not** mint a GitHub issue yet — the ticket is minted downstream, by the build path after the decision is settled.
