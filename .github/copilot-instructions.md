...
- **Deload = volume cut, intensity held.** Deload weeks cut `prescribedSets` ~40% (`applyDeloadVolume` in `js/program.js`) and append a note to prescription text; kg (intensity) is **NOT** scaled. `Loads.resolveEffective` still accepts an `isDeload` arg but ignores it (kept for call-site compat). Retest weeks are exempt from the volume cut. See `docs/adr/0003-deload-as-volume-cut.md`.
...
**Bumping the schema = bump `SCHEMA_VERSION` in `js/storage.js`, AND bump `CACHE` in `sw.js`** (e.g. `climb-planner-v14` → `v15`) so PWA clients pick up the new JS. Cache is currently at **v14**.
