...
### Build note (2026-08-15) — three corrections from the build

Shipped in `/audit-loop` pass 49. The Decision above is unchanged and was implemented as written; these are things the decision did not foresee, recorded here rather than left in a commit message.

1. **A pain-only readiness object is newly reachable, and would have restored the defect through a different door.** The addendum reasoned about the pain path only via `readinessScore`, which null-handles it correctly. But `computeReadinessMultiplier` destructures with `{ sleep = 3, soreness = 3, fatigue = 3 }` defaults, so a *truthy* `{pain:{…}}` — which the pain check-in now persists on its own, precisely because the persist half stopped fabricating s/s/f — averages 3.0 and reads **Lighter**. "Pass absent readiness straight through" is therefore not sufficient on its own. The build added a view-local `readinessForMultiplier()` returning null when **none** of sleep/soreness/fatigue is present.
   - **"None present", not "any missing"** — deliberately *looser* than `readinessScore`'s any-missing rule. The strict rule is right for a longitudinal baseline (a partial score biases a 28-day average) and wrong for same-day gating: under it, an athlete who taps sleep 1 and soreness 1 but forgets fatigue would read Normal, discarding a genuine bad report — the mirror image of the IB-058 defect. Partial check-ins therefore behave exactly as they did before this addendum.
   - This does sit in mild tension with the Decision's own rationale that the "no data → Normal" contract should live in **one** place (`js/loads.js`). It is in the view because "no load-chain change" was an explicit constraint of this decision. Relocating the test into `computeReadinessMultiplier` is the better long-term home and should be folded into the next sanctioned `js/loads.js` change (ADR-0019 / IB-064 will be editing `resolveEffective` anyway).
2. **`js/views/log.js`'s `fmtReadiness` averaged every numeric key in the readiness object**, including the `multiplier` the Today view stores alongside the pills — so `{3,3,3,multiplier:0.85}` displayed "Readiness 2.5". That was bounded while the view fabricated all three pills; once it stopped, a partial or pain-only day could render a score from a single field. Fixed to average sleep/soreness/fatigue **by name**, returning nothing unless all three are present — `readinessScore`'s strict contract, which *is* the right one there because the Log feed is a historical readout, not same-day gating.
3. **"No existing case asserts the old `{3,3,3}` behaviour, so nothing needs un-asserting" was wrong.** `[IB-041]` in `tests/cases/02-today-view.js` asserted a `readiness ×` step in the load trail, which `js/loads.js` emits only when the multiplier is not 1.0 — so that case's coverage rested on the fabricated default. Its fixture now performs an explicit check-in, testing the chain rather than the artefact.

**Out of scope, recorded so it is not re-discovered as new:** days already persisted with a fabricated `{3,3,3}` by the old `getOrInitDay` keep reading Lighter and keep sitting in the `readinessTrend` baseline. They are indistinguishable from a deliberately-logged straight-3, so no migration can separate them, and this addendum ruled out a schema bump. Separately, a *partial* check-in still receives `computeReadinessMultiplier`'s `= 3` defaults for the untapped pills — the same "fabricate a 3" mechanism at smaller scale, left alone here by the no-load-chain-change constraint and tracked as its own backlog row.
