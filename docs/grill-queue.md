...
| IB-064 | A hold week suppresses the +2.5% step (IB-028/056) but **not** the readiness "Push" (×1.05), which still raises finger/pull kg +5% on deload/retest/taper/pain-amber weeks. Should a hold also clamp `readinessMultiplier ≤ 1.0`, or is an upward Push on fingers ever wanted during recovery? | P1 | [#76](https://github.com/slm37102/simple-climbing-training-planner/issues/76) | Queued |
...
| IB-020 | The finger norm table is a **7s** hang but `maxHang20mm` is a **10s** hold, compared with no conversion — retest at 7s (breaks benchmark continuity), apply a documented conversion (another invented constant), widen the "meaningfully below" threshold, or accept it? The honest-labeling half already shipped (pass 39); only the numeric call is open. | P2 | [#77](https://github.com/slm37102/simple-climbing-training-planner/issues/77) | Queued |
