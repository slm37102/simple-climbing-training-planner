...
> local date-helper copies (carried forward as **IB-047**); **S3's** whole-state replace on
> import is now intentional behavior behind an explicit destructive confirm. Treat the findings
> below as a historical record, not a to-do list — this document is the dated 2026-06-24 audit,
> not a tracker. **New engineering findings go to [`improvement-backlog.md`](improvement-backlog.md)**
> (`IB-*`, the live ledger); training-content gaps live in [`knowledge-gaps.md`](knowledge-gaps.md).
...
| U9 | Low | **Goal/comp day renders as "Optional: easy open climb or rest"** — the final cycle day (the one date the whole plan is anchored to) falls through to the generic `sun-optional` template with no goal-day label. Cosmetic but confusing; surfaced by the 2026-07-14 coaching review (§11 `[new]`) and visible in `training-plan.md`'s generated week-12 schedule. *(Added 2026-07-15.)* | [js/program.js](../js/program.js) `prescribeForContext` (no `diffDays === totalDays - 1` branch) | Add a goal-day branch (mirror the existing `rest-pre-goal` special case at `totalDays − 2`): label it "🏁 Goal / comp day", keep an optional easy-warm-up exercise, vary copy by `peakType`. |
