...
| KG-A7 | Injury-prevention dosing partial | P2 | G3 | Open — dosing half closed (#40, 2026-07-16); return-from-tweak half open (#28) |
...
- **Dosing half CLOSED 2026-07-16 → #40.** Antagonist/shoulder work now runs 2×/week: a short ~10-minute block (`TUE_ANTAGONIST_BLOCK`, 2 exercises) was added to Tuesday's light day alongside the existing full 5-exercise Monday block (`ANTAGONIST_BLOCK`), and Monday's block is no longer dropped on deload weeks — it now goes through the same volume-cut path as every other exercise (`applyDeloadVolume`/`applyDeloadToExercise` in `js/program.js`, which appends a deload note to each item's text) instead of the old `!isDeload` guard that skipped it entirely. `[KG-A7]` regression tests added to `tests/index.html`. The **return-from-tweak protocol remains open**, tracked separately in #28 — this entry stays Open until that half closes too.
- **Return-from-tweak half DECIDED 2026-07-16 → reference doc, no app logic** (wayfinder ticket #28, grounded in the #27 evidence sweep at [`research/return-from-tweak-evidence.md`](research/return-from-tweak-evidence.md)). Every tweak-zone constant is practitioner convention and no RCT ranks protocols, so the contested numbers stay out of the load engine; daily *detection* is already in-app via ADR-0014's pain check-in, whose pain-red response links the doc. Deliverable: a `docs/` one-pager (Silbernagel gate as labelled tendon-extrapolation, open-hand de-escalation ladder, 2–5%/wk reload ramp, red-flag handoff list), cut at the map's assembly step. A future "tweak-mode" toggle stays a possible upgrade with the doc as its spec. This entry closes when the doc ships.
...
- **Checklist half shipped 2026-07-16** → [`end-of-cycle-review.md`](end-of-cycle-review.md) (issue [#43](https://github.com/slm37102/simple-climbing-training-planner/issues/43), closes KG-D7). Covers retest trajectory (incl. what two flat retests in a row means — a judgment call, ties to KG-A4's still-unbuilt "flat retest → change stimulus" rule), which protocols to rotate, when to shift phase ratios toward the evolving limiter, and the Peak-aggressiveness ratchet-up question above. **This entry stays Open** — the automation half (auto-detecting drift/limiter shift with no human read required) is unbuilt and P3. The related W13 micro-retest-cadence question (benchmarks going stale through Build/Peak, coach-review W13) was separately decided in [ticket #31](https://github.com/slm37102/simple-climbing-training-planner/issues/31) — both a post-goal retest slot and a staleness-gated Build-Monday micro-retest, feeding this checklist's inter-cycle honesty — but is **not yet implemented** (no ADR or code shipped as of this writing); the checklist references it, it does not resolve it.
...
| KG-B13 | Taper volume is cut twice | P3 | G2 | Closed (2026-07-16) |
...
- **CLOSED 2026-07-16 (issue #38).** Picked the "author at full volume" layer, per the #34 ratification. Re-authored every taper template's pre-cut numbers so `applyTaperVolume`'s single ×0.6 is the only reduction, each landing within what the template's own prescription text already argues for: taper pull-ups `prescribedSets` 2 → 4 (cuts to 2, matching "2 × 2"); `thu-projects` goes 2 → 4 (cuts to 2, within "2–3 quality goes", was shipping as 1); `thu-flash`/`sat-route-mileage` problems/routes 8 → 14 (cuts to 8, within "6–10"); `sat-volume-down` minutes 50 → 80 (cuts to 50, within "45–60 min"). `HANGBOARD.taper` needed no change (its `prescribedSets: 1` already sits below the cut function's `>1` guard, so it was never double-cut). Pluralization fixed alongside: `js/exercise-inputs.js` gained `unitLabel(value, unit)` (singularizes when `value === 1`, with an irregular-plural map for `'goes' → 'go'`), wired into `today.js`'s `targetCalloutHtml`/`accSub` and `tools/generate-schedule.mjs`'s `targetBit`. `[KG-B13]` regression tests added to `tests/index.html` (`Program.build` taper fixtures across Mon/Thu/Sat and both flavors, plus a DOM regression on a naturally-occurring singular deload-cut target); two pre-existing ADR-0007/gym-ready tests that had asserted the double-cut numbers were updated to the corrected values. `sw.js` CACHE v34→v35; schedule regenerated (week-12 rows).
...
- **2026-07-16** — **KG-B13 closed** (issue [#38](https://github.com/slm37102/simple-climbing-training-planner/issues/38)), completing the KG-B10–B13 coach-review batch. Taper templates (Mon pull-ups, Thu `thu-flash`/`thu-projects`, Sat `sat-volume-down`/`sat-route-mileage`) re-authored at full pre-cut volume so `applyTaperVolume`'s single ×0.6 cut is the only reduction, landing at what each template's own text already argues for — pull-ups 2×2 (was shipping as 1×2), project-route Thursdays 2 goes (was 1). `js/exercise-inputs.js` gained `unitLabel()` to singularize `prescribedTarget` units ("1 go", not "1 goes"), wired into `today.js` and `tools/generate-schedule.mjs`. 7 new `[KG-B13]` regression tests added, 2 pre-existing tests (`[ADR-0007]`, `[Gym-ready]`) updated to the corrected numbers — 151/151 passing. `sw.js` CACHE v34→v35; schedule regenerated (week-12 rows change).
