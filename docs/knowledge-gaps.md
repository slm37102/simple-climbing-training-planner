...
The app was built before the research; these are the places where its decisions contradict or ignore the verified findings. **KG-B1–B13 are Closed** — full detail for all thirteen is in [the archive, Lens B](knowledge-gaps-archive.md#lens-b). KG-B14, KG-B15, KG-B16 and KG-B17 are **Resolved by decision with the build still pending**; they graduated in from the improvement backlog via the grill queue, so their write-ups live in their ADRs rather than the archive.
